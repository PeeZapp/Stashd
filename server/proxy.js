...
// ── Price comparison LLM extractor ─────────────────────────

async function llmExtractPriceComparison(pageText, links, currentPrice, productTitle) {
  const apiKey = process.env.AI_INTEGRATIONS_OPENAI_API_KEY;
  const baseURL = process.env.AI_INTEGRATIONS_OPENAI_BASE_URL;
  if (!apiKey || !baseURL) return [];

  const linksJson = JSON.stringify(links.slice(0, 30));

  const prompt = `You are a shopping price comparison assistant. The user saved "${productTitle}"${currentPrice > 0 ? ` at $${currentPrice.toFixed(2)}` : ''}.

Below is text from a Google Shopping search results page, followed by links found on the page.
...
Extract up to 5 shopping results for the same or very similar product. For each result:
- store: the retailer name
- price: price as a number (e.g. 29.99)
- title: product title as shown in results
- url: pick the most relevant href from the links list that matches this result (prefer direct merchant URLs over google.com links). Use null if none match.

Return ONLY valid JSON:
{
  "results": [
    { "store": "Amazon", "price": 89.99, "title": "...", "url": "https://..." }
  ]
}

If you cannot find shopping results, return {"results": []}.`;
  ...
  try {
    ...
    return results
      .filter((r) => r.price && r.store)
      .map((r) => ({
        store: String(r.store),
        price: Math.round(parseFloat(r.price) * 100) / 100,
        title: String(r.title || productTitle),
        url: typeof r.url === 'string' && r.url.startsWith('http') ? r.url : null,
        savings: currentPrice > 0 ? Math.round((currentPrice - parseFloat(r.price)) * 100) / 100 : null,
      }))
      .filter((r) => !isNaN(r.price) && r.price > 0)
      .sort((a, b) => a.price - b.price);
  } catch (err) {
    console.error('[llm-price-comparison]', err.message);
    return [];
  }
}

// ── Find cheaper endpoint ───────────────────────────────────

app.get('/find-cheaper', async (req, res) => {
  ...
  const searchUrl = `https://www.google.com/search?q=${q}&tbm=shop&hl=en`;

  console.log(`[find-cheaper] searching for: ${title}`);

  let context = null;
  try {
    const bw = await getBrowser();
    context = await bw.newContext({
      userAgent:
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36',
      viewport: { width: 1280, height: 800 },
    ...
    const { links, pageText } = await page.evaluate(() => {
      ...
      const links = anchors
        ...
        .filter(
          (a) =>
            ...
            !a.href.includes('google.com/preferences') &&
            !a.href.includes('accounts.google') &&
            !a.href.includes('support.google')
        )
        .slice(0, 50);

      const pageText = (document.body ? document.body.innerText : '').slice(0, 6000);
    ...
    console.log(`[find-cheaper] got ${links.length} links, ${pageText.length} chars of text`);

    const results = await llmExtractPriceComparison(pageText, links, currentPrice, title);

    console.log(`[find-cheaper] found ${results.length} results`);
    res.json({ results });
  } catch (err) {
    if (context) await context.close().catch(() => {});
    console.error('[find-cheaper] error:', err.message);
