...
import { X, Plus, Trash2, Share2, ExternalLink, CreditCard as Edit2 } from 'lucide-react';
...
export default function ListsPanel({ lists, onClose, onListsChanged }: ListsPanelProps) {
  ...
  const handleShareList = async (list: List) => {
    ...
    const shareUrl = `${window.location.origin}/share/list/${list.share_token}`;
    navigator.clipboard.writeText(shareUrl);
    alert('Share link copied to clipboard!');
  };

  const getTotalCost = (listId: string) => {
    const list = lists.find((l) => l.id === listId);
    if (!list) return 0;

    if (selectedList === listId) {
      return listProducts.reduce((sum, p) => sum + p.current_price, 0);
    }
    return 0;
  };
                    ...
                    <div className="flex-1">
                      ...
                      {selectedList === list.id && !loading && (
                        <p className="text-sm text-gray-600">
                          {listProducts.length} item{listProducts.length !== 1 ? 's' : ''} · Total:{' '}
                          ${getTotalCost(list.id).toFixed(2)}
                        </p>
                      )}
                    </div>
                          ...
                          {listProducts.map((product) => (
                            ...
                            >
                              ...
                              <div className="p-2">
                                ...
                                <p className="text-xs text-gray-600">${product.current_price}</p>
                              </div>
                            </div>
                          ))}
                        </div>
